...
function referenced(manifest: Manifest) {
  ...
  for (const org of Object.values(manifest.orgs)) {
    ...
    for (const section of org.chart_sections ?? []) {
      ...
      for (const download of Object.values(section.downloads ?? {})) downloads.push(download.path);
...
export async function buildSite(): Promise<string> {
  ...
  for (const [route, body] of Object.entries(ROUTES)) {
    // A string is a raw file (a companion CSV) and goes down verbatim; quoting it
    // as JSON would hand the app a CSV wrapped in quotes. An entry with nothing
    // to serve is skipped here and left for the completeness check below to name.
    if (body === undefined) continue;
    await write(join(API_DIR, route), typeof body === 'string' ? body : JSON.stringify(body));
  }
  ...
  // Only where ROUTES supplied nothing: a companion CSV the fixture provides for
  // real should not be replaced by a stand-in.
  for (const file of downloads) {
    if (!existsSync(join(API_DIR, file))) await write(join(API_DIR, file), PLACEHOLDER_CSV);
  }
