"""Runner script for generating CODEOWNERS compliance and GitHub Actions runner reports."""
...
def get_codeowners_for_repos(client: GitHubClient, org: str, repos: list[RepositoryRecord]) -> list[CodeOwnersRecord]:
    ...
    records = [CodeOwnersRecord(repo=r.name, status=has_codeowners_file(client, org, r.name)) for r in repos]

    save_records_cache(kind=kind, scope=ORG, parameters=parameters, record_type=CodeOwnersRecord, records=records)
...
def generate_codeowners_markdown_report(records: list[CodeOwnersRecord], output_path: str) -> None:
    ...
    lines = ["# CODEOWNERS Compliance Report", "", "| Repository | Status |", "| :--- | :---: |"]
...
def generate_runner_markdown_report(records: list[RunnerRecord], output_file: str) -> None:
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            "This report categorizes runners into **Self-Hosted**, **Standard (GitHub-hosted)**, and **Indeterminate (Dynamic)**.\n\n"
        )
